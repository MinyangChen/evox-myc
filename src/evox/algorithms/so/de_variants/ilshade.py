...
@jit_class
class ILSHADE(Algorithm):
    """
    J. Brest, M. S. Maučec and B. Bošković, "iL-SHADE: Improved L-SHADE algorithm for single objective real-parameter optimization," 
    2016 IEEE Congress on Evolutionary Computation (CEC), Vancouver, BC, Canada, 2016, pp. 1188-1195, doi: 10.1109/CEC.2016.7743922.
    """
    ...
    def ask(self, state):
        ...
        def case_0(): return 0.7
        def case_1(): return 0.8
        def case_2(): return 0.9
        def case_3(): return 1.0

        F_max = jax.lax.cond(state.progress <= 0.25,
                            case_0,
                            lambda: jax.lax.cond(state.progress <= 0.5,
                                                case_1,
                                                lambda: jax.lax.cond(state.progress <= 0.75,
                                                                    case_2,
                                                                    case_3)))

        F_vect = jnp.where(F_vect < F_max, F_vect, F_max)
        ...
        def case_0(): return 0.5
        def case_1(): return 0.25
        def case_2(): return 0.0

        CR_min = jax.lax.cond(state.progress <= 0.25,
                            case_0,
                            lambda: jax.lax.cond(state.progress <= 0.5,
                                                case_1,
                                                case_2))
